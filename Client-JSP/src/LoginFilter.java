...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain fc) throws IOException, ServletException {
        ...
        if (req.getSession().getAttribute("beans") == null) { //checks if there's a LOGIN_USER set in session...
            res.sendRedirect(contextPath + "/LoginServlet"); //or page where you want to redirect
        }
        else {

            if (req.getSession().getAttribute("rooms") == null) {
                List<Room> rooms = remoteRoom.getUserRooms(((User)req.getSession().getAttribute("beans")).getId());
